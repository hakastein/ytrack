...
Тесты идут против фейкового сервера и живого YouTrack не требуют. Спеку `make openapi` снимает с
локального YouTrack — дев-инстанса с проектом `DEV`, на котором заведены кастом-поля всех типов. На нём же
проверяют руками и измеряют факты о сервере для ADR. Поднимается он из `dev/` одной командой, из зависимостей
