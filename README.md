...
make generate   # перегенерировать каталог схем по OpenAPI-спеке модуля youtrack
make ytapi      # каталог воспроизводится из спеки, сгенерированный клиент зовёт только адаптер
```

Транспорт, сгенерированный клиент, типы кастом-полей и фейковый сервер ytrack берёт из модуля
[`github.com/hakastein/youtrack`](https://github.com/hakastein/youtrack). Тесты идут против фейкового сервера и живого
YouTrack не требуют. Локальный YouTrack — дев-инстанс с проектом `DEV`, на котором заведены кастом-поля всех типов:
с него `make openapi` модуля снимает спеку, на нём проверяют руками и измеряют факты о сервере для ADR. Поднимается он из `dev/` одной командой, из зависимостей
