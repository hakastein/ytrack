...
// countIssuesWithRetry asks the counter once more, straight away, where it answered -1, and once is all: an instance
...
// countIssues is how many issues the search finds, which the server counts itself: a second page over ids alone
...
// hasIssueBlocks is whether an answer of that schema was built by one of those places.
...
func issueBlocks(spec *schemas, at blockSchema, asked []requestedField) {
	...
	translated := hasDefaultNames(spec, asked)
...
// rejectIssueBlocks is what an expression may not write where issueBlocks composes: a name the caller puts under
...
// issueRequest is one request for issues, whatever command asks it: the expression that goes out and the names
...
// customFieldsFilter is the names the request itself is narrowed by, one customFields= parameter each. The server
